...
% 4) Run PCA per scale.
% 5) Concatenate per-scale scores into one unified chunk embedding table.
% 6) Optionally run a second-stage global PCA over concatenated scale scores.
%
% Important note
% --------------
% This is intentionally simple and interpretable. It is a scientifically
% defensible baseline before temporal autoencoders / contrastive encoders.
%
% Inputs
% ------
...
% Name-value pairs
% ----------------
% 'nPCsPerScale'        : max PCs retained per scale (default 12)
...
%   Final chunk embedding matrix after optional global PCA.
...
for s = 1:nScale
    ...
    if P.verbose
        fprintf('Embedding scale %d/%d (%.3gs) | nChunks = %d | L = %d | D = %d\n', ...
    ...
    if nChunks < P.minChunksPerScale
        warning('fit_multiscale_chunk_embedding:TooFewChunks', ...
            'Skipping scale %.3gs because nChunks=%d < minChunksPerScale=%d.', ...
    ...
    if maxPC < 1
        warning('fit_multiscale_chunk_embedding:NoPCs', ...
            'Skipping scale %.3gs because there are too few usable dimensions.', Sc.chunkSec);
        allTables{s} = table();
        allChunkIds{s} = zeros(0,1);
        allScores{s} = zeros(0,0);
        allNames{s} = strings(0,1);
...
[embedding, embeddingNames, globalModel] = build_global_embedding(chunkTable, allNames, P.globalNPCs);
...
% Invalidate all channels at invalid frames.
for t = 1:L
    bad = ~V(:,t);
    if any(bad)
        X(bad,t,:) = NaN;
...
for t = 1:L
    for d = 1:D
        ...
        family(idx) = string(ChunkSet.featureMeta.Family{strcmp(ChunkSet.featureNames, channelMeta.BaseFeature(d))});
        timeIndex(idx) = t;
    end
end
...
function weights = build_chunk_feature_weights(dimMeta, channelMeta, familyWeightsStruct, booleanWeight)
...
% Normalize mean weight to 1 for stability.
muw = mean(weights);
...
% Drop all-zero columns only if all are not zero.
...
function chunkTable = join_scale_embeddings(baseChunkTable, allTables, allNames)
...
function [embedding, embeddingNames, globalModel] = build_global_embedding(chunkTable, allNames, globalNPCs)
scoreNames = strings(0,1);
for s = 1:numel(allNames)
    scoreNames = [scoreNames; allNames{s}(:)]; %#ok<AGROW>
end
...
if isempty(scoreNames)
    ...
    globalModel = struct('coeff', [], 'explained', [], 'scoreNames', scoreNames);
    return
end
...
[coeff, score, ~, ~, explained] = pca(X, 'NumComponents', nPC);
embedding = score;
