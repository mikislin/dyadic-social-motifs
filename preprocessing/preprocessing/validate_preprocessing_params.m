...
assert(P.smooth.low_pass_frq < P.data.fps/2, ...
    'P.smooth.low_pass_frq must be below the Nyquist frequency');
...
if ~isempty(P.smooth.low_pass_frq_by_node)
    ...
    assert(all(P.smooth.low_pass_frq_by_node < P.data.fps/2), ...
        'P.smooth.low_pass_frq_by_node values must be below the Nyquist frequency');
end
