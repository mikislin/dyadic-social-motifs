...
Scale = repmat(struct( ...
    ...
    'channelMeta', table(), ...
    'featureNames', {{}}, ...
    'featureMeta', table()), numel(chunkSec), 1);
...
for s = 1:numel(chunkSec)
    ...
    Scale(s).obsNames = obsNames;
    Scale(s).channelMeta = channelMeta;
    Scale(s).featureNames = seqs{1}.featureNames;
    Scale(s).featureMeta = seqs{1}.featureMeta;
    if ~isempty(metaRows)
